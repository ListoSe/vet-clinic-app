...
export default function MainAppAdmin({ user, onLogout }: AdminProps) {
  ...
  const renderTab = () => {
    switch (tab) {
      case 'vets':
        return <VetList currentUser={user} />;
      case 'owners':
        return <OwnersList currentUser={user} />;
      case 'animals':
        return <PetList currentUser={user} />;
      case 'records':
        return <RecordsList currentUser={user} />;
      case 'finance':
        return <FinanceList currentUser={user} />;
      default:
        return null;
    }
  };
    ...
    >
      ...
      >
        ...
        <nav style={{ flex: 1 }}>
          ...
          <div
            className={`nav-item ${tab === 'owners' ? 'active' : ''}`}
            onClick={() => setTab('owners')}
          >
            👥 Власники
          </div>
          <div
            className={`nav-item ${tab === 'animals' ? 'active' : ''}`}
          ...
          <div
            className={`nav-item ${tab === 'finance' ? 'active' : ''}`}
