...
export default function OwnersList({ currentUser }: OwnersListProps) {
  const [owners, setOwners] = useState<Owner[]>([
    {
      ...
      pets: [{ name: 'Бакс', species: 'собака' }, { name: 'Мурка', species: 'кіт' }]
    },
    {
      id: 2,
      name: 'Олена Іванова',
  ...
  const handleSave = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingOwner) {
      setOwners(owners.map(o => o.id === editingOwner.id ? { ...o, ...ownerData } : o));
    } else {
      ...
      <table className="data-table">
        ...
        <tbody>
          {filteredOwners.map((o) => (
            <tr key={o.id} className="clickable-row" onClick={() => { setEditingOwner(o); setIsModalOpen(true); }}>
              <td>
              ...
              <td>
                {o.pets.length > 0 ? (
                  ...
                  <span style={{ color: '#94a3b8', fontSize: '12px' }}>Немає тварин</span>
                )}
              </td>
        ...
        <div className="modal-overlay" onClick={() => setIsModalOpen(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <h3 style={{ marginTop: 0, marginBottom: '20px' }}>
              {!isAdmin ? 'Картка власника' : editingOwner ? 'Редагувати власника' : 'Нова картка власника'}
            </h3>
            <form onSubmit={handleSave}>
              <label className="input-label">ПІБ Власника</label>
              <input name="name" defaultValue={editingOwner?.name} className="input-field" required readOnly={!isAdmin} />

              <label className="input-label">Контактний телефон</label>
              <input name="phone" defaultValue={editingOwner?.phone} className="input-field" required readOnly={!isAdmin} />

              <label className="input-label">Email</label>
              <input name="email" defaultValue={editingOwner?.email} className="input-field" required readOnly={!isAdmin} />
                  ...
                  <div style={{ padding: '10px', backgroundColor: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                    {editingOwner.pets.map((p, i) => (
                      <div key={i} style={{ fontSize: '14px', marginBottom: '4px' }}>
                        {p.species === 'собака' ? '🐕' : '🐈'} <strong>{p.name}</strong> ({p.species})
                      </div>
                    ))}
                  </div>
                </div>
